<?php $__env->startSection('subhead'); ?>
    <title>Chat</title>
    <style>
        .clear-both {
            clear: both
        }

        .bg-primary {
            background-color: #074e62 !important
        }
    </style>
<?php $__env->stopSection(); ?>

<?php $__env->startSection('subcontent'); ?>
    <div class="loader"></div>
    <div class="intro-y flex flex-col sm:flex-row items-center mt-8">
        <h2 class="text-lg font-medium mr-auto">Chat</h2>
    </div>
    <div class="intro-y chat grid grid-cols-12 gap-5 mt-5">
        <!-- END: Chat Side Menu -->
        <!-- BEGIN: Chat Content -->
        <div class="intro-y col-span-12 lg:col-span-12 2xl:col-span-12">
            <div class="chat__box box">
                <!-- BEGIN: Chat Active -->
                <div class=" h-full flex flex-col">
                    <div class="flex flex-col sm:flex-row border-b border-slate-200/60 dark:border-darkmode-400 px-5 py-4">
                        <div class="flex items-center">
                            <div class="w-10 h-10 sm:w-12 sm:h-12 flex-none image-fit relative">
                                <img class="rounded-full" style="width: 100%; height: 100%;" src="/<?php echo e($data['userProfile']); ?>"
                                    onerror="this.onerror=null;this.src='/build/assets/images/person.png';"
                                    alt="Astrologer image" />
                            </div>
                            <div class="ml-3 mr-auto">
                                <div class="font-medium text-base"><?php echo e($data['userName']); ?></div>
                            </div>
                        </div>
                        <div
                            class="flex items-center sm:ml-auto mt-5 sm:mt-0 border-t sm:border-0 border-slate-200/60 pt-3 sm:pt-0 -mx-5 sm:mx-0 px-5 sm:px-0">
                        </div>
                    </div>
                    <div class="overflow-y-scroll  px-5 pt-5 flex-1" style="overflow-y:auto " id="chatarea">
                       
                    </div>
                    <form method="POST" enctype="multipart/form-data" style="width:100%" id="create-chat">
                        <div
                            class="ml-3 pt-4 pb-10 sm:py-4 flex items-center border-t border-slate-200/60 dark:border-darkmode-400">
                            <?php if($data['ticketStatus'] != 'CLOSED'): ?>
                                <?php echo csrf_field(); ?>
                                <input type="hidden" name="messageCount" value="<?php echo e(count($messages)); ?>">
                                <input type="hidden" name="chatId" value="<?php echo e($data['chatId']); ?>">
                                <input type="hidden" name="senderId" value="<?php echo e($data['userId']); ?>">
                                <input type="hidden" name="ticketId" value="<?php echo e($data['ticketId']); ?>">
                                <input type="hidden" name="ticketStatus" value="<?php echo e($data['ticketStatus']); ?>">
                                <textarea
                                    class="chat__box__input form-control dark:bg-darkmode-600 h-16 resize-none border-transparent px-5 py-3 shadow-none focus:border-transparent focus:ring-0"
                                    rows="1" id="message" name="message" placeholder="Type your message..."></textarea>
                                <div style="position:static"
                                    class="flex absolute sm:static left-0 bottom-0 ml-5 sm:ml-0 mb-5 sm:mb-0">
                                </div>
                                <button type="submit"
                                    class="w-8 h-8 sm:w-10 sm:h-10 block bg-primary text-white rounded-full flex-none flex items-center justify-center mr-5">
                                    <i data-lucide="send" class="w-4 h-4"></i>
                                </button>
                            <?php endif; ?>
                        </div>
                    </form>
                </div>
            </div>
        </div>
    </div>
    </div>
<?php $__env->stopSection(); ?>
<?php $__env->startSection('script'); ?>
    <script type="text/javascript">
    get_chat();
    setInterval(function(){
        get_chat();
    }, 1000);

    var spinner = $('.loader');
        jQuery('#create-chat').submit(function(e) {
            e.preventDefault();
            // spinner.show();
            jQuery.ajax({
                type: 'POST',
                url: "<?php echo e(route('createChat')); ?>",
                data: new FormData(this),
                dataType: 'JSON',
                processData: false,
                contentType: false,
                success: function(data) {
                    if (jQuery.isEmptyObject(data.error)) {
                        // spinner.hide();
                        $('#message').val('');
                        get_chat();
                    } else {
                        spinner.hide();
                    }
                }
            });
        });
        $(window).on('load', function() {
            $('.loader').hide();
        })

        function get_chat(){
            var CSRF_TOKEN = $('meta[name="csrf-token"]').attr('content');
            jQuery.ajax({
                url: "<?php echo e(route('getChat')); ?>",
                type: 'POST',
                data: {_token: CSRF_TOKEN, id:"<?php echo e($data['ticketId']); ?>"},
                success: function(data) {
                    $('#chatarea').html(data);
                }
            });
        }
    </script>
<?php $__env->stopSection(); ?>

<?php echo $__env->make('../layout/' . $layout, \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH /home2/drodifyt/astromatch.drodifytechnology.com/resources/views/pages/chat.blade.php ENDPATH**/ ?>