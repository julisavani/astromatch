...
class ChatController extends Controller
{
    ...
    public function getFireStoredata(Request $req)
    {
        try {
            ...
            $response = $httpClient->get("https://firestore.googleapis.com/v1/projects/astromatch-be7e5/databases/(default)/documents/supportChat/{$chatId}/userschat/{$req->id}/messages");
    ...
    public function getChat(Request $req)
    {
        try {
            ...
            if (isset($apiResponse['documents'])) {
                ...
                return view('pages.chat_live', compact('messages', 'data'))->render();
            } else {
                // Handle the case when there are no documents
                $messages = [];
                return view('pages.chat_live', compact('messages', 'data'))->render();
    ...
    {
        try {
            $apiEndpoint = "https://firestore.googleapis.com/v1/projects/astromatch-be7e5/databases/(default)/documents/";
